...
- **Node:** `app.panel` (id / title / icon / dock left|right|bottom) registers a docked panel; the plugin builds its contents in an On App Event flow (event=panel, id=this panel) using the ordinary `ui.*` nodes drawing into a window whose id is the panel id. Buttons / sliders / inputs inside fire `ui.on` (the existing UI-event family) - no new interaction surface.
...
- **In-process scenes:** `AppSink` fulfils `UiWindow/UiUpsert/UiAnimate/UiRemove/UiClose` by building `UiScene`s on the `PluginHost` (no child process). `MainScreen` reconciles a `DockManager` panel per contribution, paints each with the violet App-category card chrome, and routes panel `UiEvent`s back through `PluginManager.PanelEvent` -> the plugin's `ui.on`. (Scene access is guarded by `PluginManager.SceneGate` since flows run off-thread - see Phase 6.)
...
- **Trust registry (`plugins/registry.json`):** consent + on/off state are **persisted**, and consulted by `LoadInstalled`. A plugin only auto-loads if it's recorded as approved *and* asks for nothing beyond what was approved (`PermsApproved`); otherwise it loads **Pending** - present but inert. So a file dropped into the folder does NOT run on next launch: it's held for review (surfaced on launch via the trust card + in the manager). Disabling persists across restarts; a swapped-in file that wants a new permission is re-held for review; uninstall forgets the approval.

**Phase 6: plugin flows run off the UI thread - COMPLETE.**
- Each `PluginHost` runs its flows on its **own worker thread** (a `BlockingCollection<Action>`, like `ServerConn`), so a blocking node (`container.exec`, `net.http`, a delay) never freezes the editor. One thread per plugin serialises that plugin's flows, so its `State` / `Scenes` are never touched by two flows at once.
- App-facing effects marshal to the UI thread: `MainScreen` drains a `ConcurrentQueue<Action>` each frame; void effects (toast / nav / bot / dialog / confirm / graph edits) post-and-return, value effects (`Info` / `GraphEdit`) post-and-block-briefly (`InvokeOnUi`, no deadlock since no flow runs on the UI thread). Scene reads/writes are guarded by `PluginManager.SceneGate`, held briefly by `MainScreen` while it renders a panel and by the worker on each `ui.*` op (never across a blocking node).
- `app.confirm` pause/resume already fit this: the worker flow ends at the modal; the UI-thread answer re-schedules `FireFrom` on the worker.
- Tests stay synchronous (`PluginManager(app, threaded: false)` runs inline); `PluginThreadTest` covers the off-thread path via `WaitIdle`.

**Phase 7: plugin settings + secret picker - COMPLETE.**
- **Nodes:** `app.settings` declares one form field (key / label / type=text|password|secret); `app.opensettings` opens the modal (wire a gear button's `ui.on` into it). Each field's saved value is exposed as a `{key}` token the plugin's flows resolve.
- **Settings modal:** `MainScreen` renders the declared fields - `Ui.TextField` for text/password, and for **secret** fields a "Choose a key" button that opens the existing **secret picker** (`OpenSecretPicker`), storing a `{{secret.NAME}}` reference. Save persists to `plugins/config.json`; `IAppHost.OpenSettings` marshals to the UI thread; debug `--showpluginsettings`.
- **Persistence:** settings are seeded into the plugin's state on enable (so `{tokens}` resolve before it runs) and re-applied live on save.
- **Wiring it to AI:** `ai.reply`'s `apiKey` now resolves `{var}` tokens (a settings var holding a `{{secret.NAME}}` ref expands to the key) - backward compatible. `ui.text`/`ui.button` gained an `icon` flag (render the label as a Phosphor icon, e.g. a gear).
- **Tests:** `PluginSettingsTest` (field declared, saved value resolves as a token, `app.opensettings` fires, persists across reload).

**The plugin system is feature-complete:** all four hook surfaces, all four powers (dialogs/toasts, own panel, app state + navigation, act on the open bot), **plugin settings** (form + secret picker), authoring by bundling a workflow, `.ircplugin` distribution with a trust card, a manager, and **off-thread execution** so even a slow terminal command keeps the UI live. Future work is ecosystem (a community-plugins repo + in-app gallery), not core.

**Samples** (`docs/examples/`): `pilot.ircplugin` (toolbar + commands using the state/nav/bot powers), `pilot-panel.ircplugin` (a right-docked status panel), `terminal.ircplugin` (a **bottom-docked terminal** - type a command, it runs in an isolated alpine container via `container.start`/`container.exec` and prints the output; needs Docker/Podman; the command echoes + the prompt clears immediately, then runs off-thread so the editor stays live), and `ai-assistant.ircplugin` (an **AI chat side panel** - talk to any OpenAI-compatible model; it can view + edit your workflows via `ai.editor`; a gear opens a **settings modal** for base URL / model / API key, the key chosen from your saved secrets). `app.panel` now also docks `bottom`, and `ui.input` gained `font` / `size` params (so the terminal renders in mono); `ui.text`/`ui.button` gained an `icon` flag.

**Responsive panels:** a panel is (re)built whenever its dock rect changes, and the build flow receives `{app_panel_w}` / `{app_panel_h}`; `ui.*` position/size params (`x`/`y`/`w`/`h`) resolve `{tokens}`, so a panel can size itself to fill (use `data.math` on `{app_panel_h}` to pin an input to the bottom and let a transcript fill the rest). `ui.input` also gained **`readonly`** (display only - no focus/typing, and no box chrome: just text on the panel, for a transcript), **`placeholder`** (a dim hint), and **`textColor`** (so a transcript reads on a light or dark panel). A chat output can't be accidentally typed into, and the real input field is obvious.

**Live updates from a slow node:** `UiSetText(window, element, text)` cheaply updates one element's text mid-run; because a panel re-renders every frame (and flows are off-thread), the change shows immediately. `ai.reply` uses it: set its `statusElement` (+ `statusWindow`) and it streams **`Thinking…` → `Using <tool>…` per tool → cleared** into that element live, so you watch the AI work. It also raised the tool-loop cap to a configurable **`maxSteps`** (default 12), records the tools it called into `{ai_tools_used}`, and writes a `{ai_error}` note when a turn yields no usable reply (loop exceeded / empty) so a chat never shows a blank turn. `ui.input` single lines now scroll to keep the caret visible. The AI Assistant sample uses all of this on a soft cozy-light panel.
