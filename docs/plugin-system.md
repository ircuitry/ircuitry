...
**Samples** (`docs/examples/`): `pilot.ircplugin` (toolbar + commands using the state/nav/bot powers), `pilot-panel.ircplugin` (a right-docked status panel), and `terminal.ircplugin` (a **bottom-docked terminal** - type a command, it runs in an isolated alpine container via `container.start`/`container.exec` and prints the output; needs Docker/Podman; the command echoes + the prompt clears immediately, then runs off-thread so the editor stays live). `app.panel` now also docks `bottom`, and `ui.input` gained `font` / `size` params (so the terminal renders in mono).
