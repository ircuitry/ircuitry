...
**AI chat niceties:** because flows run off-thread, a panel can show a "thinking…" indicator by setting a `{status}` var the transcript renders *before* the (blocking) `ai.reply` node runs, then clearing it after. And `ai.reply` now records the tools it called this turn into `{ai_tools_used}` (a ready-to-show note like `(used get_graph, set_param)`, or empty), so the panel can show what the AI did. The AI Assistant sample uses both, on a soft cozy-light panel.
