...
public partial class MainScreen
{
    ...
    private const float BakePop = 0.85f;   // bot rises out
    private const float BakeHold = 0.5f;
    ...
    private void DrawBakeAnim(Renderer r, Clock clock)
    {
        ...
        // ---- the oven ----
        r.Begin(BlendMode.Alpha);
        var body = new RectF(cx - 92, cy - 64, 184, 150);
        r.RoundFill(new RectF(body.X, body.Y + 4, body.W, body.H), Theme.WithAlpha(Color.Black, 0.10f * a), 16f);  // shadow
        r.RoundFill(body, Theme.WithAlpha(Theme.Mix(Theme.PanelHi, Theme.Amber, 0.18f), a), 16f);
        r.RoundOutline(body, Theme.WithAlpha(Theme.Edge, a), 16f);
        // control strip + knobs
        var strip = new RectF(body.X + 12, body.Y + 12, body.W - 24, 22);
        r.RoundFill(strip, Theme.WithAlpha(Theme.PanelLo, a), 8f);
        for (int i = 0; i < 3; i++)
        {
            var kp = new Vector2(strip.X + 16 + i * 24, strip.Center.Y);
            r.Disc(kp, 6f, Theme.WithAlpha(Theme.Panel, a));
            r.Disc(kp, 6f, Theme.WithAlpha(Theme.Edge, a * 0.5f));
            float kn = t * (2.2f + i) ;
            r.Line(kp, kp + new Vector2(MathF.Cos(kn), MathF.Sin(kn)) * 4.5f, Theme.WithAlpha(Theme.TextDim, a), 1.6f);
        }
        // oven door + window
        var door = new RectF(body.X + 14, body.Y + 44, body.W - 28, body.H - 58);
        r.RoundFill(door, Theme.WithAlpha(Theme.Mix(Theme.PanelLo, Theme.Edge, 0.3f), a), 12f);
        var win = new RectF(door.X + 14, door.Y + 12, door.W - 28, door.H - 34);
        r.RoundFill(win, Theme.WithAlpha(Theme.Mix(Theme.Amber, Theme.AmberBright, breath), 0.85f * a), 10f);
        r.RoundOutline(win, Theme.WithAlpha(Theme.AmberDim, a), 10f);
        // door handle bar
        r.RoundFill(new RectF(door.X + 10, door.Bottom - 14, door.W - 20, 6), Theme.WithAlpha(Theme.Edge, a), 3f);
        r.End();
        // window glow (the baking warmth)
        r.Begin(BlendMode.Add);
        ...
        r.End();
        r.Begin(BlendMode.Alpha);
        r.Disc(clockPos, cr, Theme.WithAlpha(Theme.PanelHi, a));
        r.Ring(clockPos, cr, Theme.WithAlpha(Theme.CyanDim, a));
        for (int i = 0; i < 12; i++) { float ang = i * MathF.PI / 6f; r.Disc(clockPos + new Vector2(MathF.Cos(ang), MathF.Sin(ang)) * (cr - 4f), 1.1f, Theme.WithAlpha(Theme.TextDim, a)); }
        r.Line(clockPos, clockPos + new Vector2(MathF.Cos(t * 3.2f - MathF.PI / 2), MathF.Sin(t * 3.2f - MathF.PI / 2)) * (cr - 8f), Theme.WithAlpha(Theme.Text, a), 2.2f);     // minute
        r.Line(clockPos, clockPos + new Vector2(MathF.Cos(t * 0.8f - MathF.PI / 2), MathF.Sin(t * 0.8f - MathF.PI / 2)) * (cr - 12f), Theme.WithAlpha(Theme.Text, a), 2.6f);   // hour
        r.Disc(clockPos, 2.2f, Theme.WithAlpha(Theme.Berry, a));
        r.End();

        // ---- the bot pops out after the bake ----
        float popT = Clamp01((t - BakeBake) / BakePop);
        if (popT > 0f)
        {
            float pop = EaseOutBack(popT);
            float rise = (1f - popT) * 30f;
            var bp = new Vector2(cx, win.Center.Y - 6 - 70f * pop + rise);
            // twinkles
            r.Begin(BlendMode.Add);
            for (int i = 0; i < 7; i++)
            {
                float ta = t * 2f + i * (MathF.PI * 2f / 7f);
                var tp = bp + new Vector2(MathF.Cos(ta), MathF.Sin(ta)) * (40f + 8f * MathF.Sin(t * 6f + i));
                float tw = (0.5f + 0.5f * MathF.Sin(t * 7f + i * 1.7f)) * pop;
                r.Disc(tp, 1.8f * tw, Theme.WithAlpha(Theme.AmberBright, 0.8f * a * tw));
            }
            r.Glow(bp, 70f * pop, Theme.WithAlpha(Theme.CyanDeep, 0.35f * a));
            r.End();
            r.Begin(BlendMode.Alpha);
